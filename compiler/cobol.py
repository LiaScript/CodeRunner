...
def run(cmd: str, cwd: str) -> Response:
    '''
    Run the GNU-Cobol gobc compiler with the given command within the current working directory.
    '''
...
def find_all(pattern: str, output: str) -> List[Tuple[str, str, str]]:
    ...
    return re.findall("([^:\\s]+):(\\d+): "
                      + pattern +
                      ": ([^\n]+)", output)

def find_warnings(output: str) -> List[Problem]:
    '''
    Detect all warnings within the gobc output.
    '''
